 ...
 */
namespace Omega\Cyberkonsultant;
...
class DataObject implements \ArrayAccess
{
     ...
     * @return $this
     */
    public function setDataRef($key, $value = null)
    {
        if ($key === (array)$key) {
            $this->_data = $key;
        } else {
     ...
     * @return mixed
     */
    public function getDataRef($key = '', $index = null)
    {
        if ('' === $key) {
            return $this->_data;
        }
    ...
    {
        if (empty($format)) {
            $result = implode(', ', $this->getDataRef());
        } else {
            preg_match_all('/\{\{([a-z0-9_]+)\}\}/is', $format, $matches);
            foreach ($matches[1] as $var) {
                $format = str_replace('{{' . $var . '}}', $this->getDataRef($var), $format);
            }
            $result = $format;
        }
        return $result;
    ...
    public function __call($method, $args)
    {
        switch (substr($method, 0, 3)) {
            case 'get':
                ...
                return $this->getDataRef($key, $index);
            case 'set':
                $key = $this->_underscore(substr($method, 3));
                $value = isset($args[0]) ? $args[0] : null;
                return $this->setDataRef($key, $value);
            case 'uns':
                $key = $this->_underscore(substr($method, 3));
